...
class SubtitleTab(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        count_label = ctk.CTkLabel(list_actions_frame, textvariable=self.controller.subtitle_file_count_text)
        ...
        self.direction_combo = ctk.CTkComboBox(options_frame, variable=self.controller.subtitle_conversion_direction, state="readonly", width=300)
    ...
    def update_language(self):
        ...
        options = list(self._("subtitle_conversion_options").keys())
        self.direction_combo.configure(values=options)
        if self.controller.subtitle_conversion_direction.get() not in options:
            self.controller.subtitle_conversion_direction.set(options[0])

    def add_files_dialog(self):
        paths = filedialog.askopenfilenames(
            title=self._("select_subtitle_files"),
            filetypes=[(self._("subtitle_file_type"), "*.srt *.ass")]
        )
    ...
    def add_paths_to_queue(self, paths):
        for path in paths:
            if (path.lower().endswith('.srt') or path.lower().endswith('.ass')) and path not in self.controller.subtitle_file_queue:
                self.controller.subtitle_file_queue.append(path)
                self.file_listbox.insert(tk.END, path)
        self.controller.update_subtitle_ui_state()

    def remove_selected_files(self):
        selected_indices = self.file_listbox.curselection()
        for i in sorted(selected_indices, reverse=True):
            self.file_listbox.delete(i)
            del self.controller.subtitle_file_queue[i]
        self.controller.update_subtitle_ui_state()

    def update_ui_state(self):
        count = len(self.controller.subtitle_file_queue)
        self.controller.subtitle_file_count_text.set(self._("total_files").format(count))
    ...
    def set_lock(self, locked):
        ...
        self.remove_button.configure(state="normal" if len(self.controller.subtitle_file_queue) > 0 and not locked else "disabled")
